...
void calcVertexNormals(const Triangle* pTriangleArray, int numTriangles, Vertex* pVertices, int numVertices)
{
	...
	// [..code..]
	// bc we want to iterate through all faces
	for (int vtxIdx = 0; vtxIdx < numVertices; vtxIdx++) {
		// set stuff to zero, so we can start adding stuff up at zero
		for (int i = 0; i < 3; i++) {
			// pVertices[vtxIdx] = 0 ???
		}
	}
...
bool loadMeshObject(const string& meshFileName)
{
	...
	// this is about computing normals 
	// until now we only see gray. The normal does not know how the light
	// is reflected on the surface. 

	// 1. compute normals for triangle surfaces
	// 2. compute normals for vertices
	...
	// [..code..] // we have data, but vertex shader doesn't have it yet
	location = glGetAttribLocation(g_shaderProgram, "position");
	assert(location != -1);
	glVertexAttribPointer(location, 4, GL_FLOAT, ) // look at master sol?

		// there is just one buffer for the whole program
...
void displayFunc(void)
{
	...
	//===============================================================
	// [..code..]
	// this is about finding a point of interest. 
	// need to define a uniform inside a shader file, because it is a global constant and is
	// the same for all vertices. 

	// distance = dot product of difference? sth itself?

	// use mix function to mix between white and c...
