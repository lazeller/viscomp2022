...
void ETHTower::draw(World &world, DrawPass pass)
{
	...
	// SOLUTION
	TextureNormalMaterial *tn_mat = dynamic_cast< TextureNormalMaterial* >(world.getMaterial("TextureNormal"));
	assert(tn_mat != NULL);
	// ~SOLUTION

	// SOLUTION
	tn_mat->setDiffuseSpecCoef(0.6f, 0.9f, 0.2f);

	//textnorm_metalgrip
	tn_mat->setTexture(m_textures["grip"]);
	tn_mat->setNormalMap(m_textures["grip_normal"]);
 	tn_mat->use(m_modelMatrix, world);
	drawGroup(findGroup("textnorm_metalgrip"));

	//textnorm_darkiron
	tn_mat->setTexture(m_textures["dark"]);
	tn_mat->setNormalMap(m_textures["dark_normal"]);
	tn_mat->use(m_modelMatrix, world);
	drawGroup(findGroup("textnorm_darkiron"));

	//textnorm_brushedmetal
	tn_mat->setTexture(m_textures["metal"]);
	tn_mat->setNormalMap(m_textures["metal_normal"]);
	tn_mat->use(m_modelMatrix, world);
	drawGroup(findGroup("textnorm_brushedmetal"));

	//textnorm_ventmetal
	tn_mat->setTexture(m_textures["ventMetal"]);
	tn_mat->setNormalMap(m_textures["ventMetal_normal"]);
	tn_mat->use(m_modelMatrix, world);
	drawGroup(findGroup("textnorm_ventmetal"));

	tn_mat->setDiffuseSpecCoef(0.9f, 0.1f, 0.3f);

	//textnorm_facade
	tn_mat->setTexture(m_textures["facade"]);
	tn_mat->setNormalMap(m_textures["facade_normal"]);
	tn_mat->use(m_modelMatrix, world);
	drawGroup(findGroup("textnorm_facade"));

	//textnorm_concrete
	tn_mat->setTexture(m_textures["concrete"]);
	tn_mat->setNormalMap(m_textures["concrete_normal"]);
	tn_mat->use(m_modelMatrix, world);
	drawGroup(findGroup("textnorm_concrete"));

	//textnorm_asphalt
	tn_mat->setTexture(m_textures["asphalt"]);
	tn_mat->setNormalMap(m_textures["asphalt_normal"]);
	tn_mat->use(m_modelMatrix, world);
	drawGroup(findGroup("textnorm_asphalt"));

	//textnorm_vent
	tn_mat->setTexture(m_textures["vent"]);
	tn_mat->setNormalMap(m_textures["vent_normal"]);
	tn_mat->use(m_modelMatrix, world);
	drawGroup(findGroup("textnorm_vent"));
	// ~SOLUTION
