...
// SOLUTION
	vec3 normalMap = texture2D(txtNormal, tex_coords).rgb;
	normalMap = normalize(normalMap * 2.0 - 1.0);

	vec3 normal = normalize(world_tang)*normalMap.r + normalize(world_bitang)*normalMap.g + normalize(world_normal)*normalMap.b;
	normal = normalize(normal);
// ~SOLUTION
