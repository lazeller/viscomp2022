...
void calcVertexNormals(const Triangle * pTriangleArray, int numTriangles, Vertex * pVertices, int numVertices)
{
	...
	{
		for (int i = 0; i < 3; i++)
			pVertices[vtxIdx].normal[i] = 0.0f;
	}

	// For each vertex, sum up normals of adjacent triangles
	for (int triIdx = 0; triIdx < numTriangles; triIdx++)
	{
		// Get pointer to triangle normal
		const GLfloat* pTriNorm = pTriangleArray[triIdx].normal;

		// Add triangle normal to all adjacent vertices
		for (int i = 0; i < 3; i++)
		{
			pVertices[pTriangleArray[triIdx].vtxIndexA].normal[i] += pTriNorm[i];
			pVertices[pTriangleArray[triIdx].vtxIndexB].normal[i] += pTriNorm[i];
			pVertices[pTriangleArray[triIdx].vtxIndexC].normal[i] += pTriNorm[i];
		}
	}
	...
	for (int vtxIdx = 0; vtxIdx < numVertices; vtxIdx++)
	{
		...
		GLfloat* pVtxNorm = pVertices[vtxIdx].normal;

		// Normalize vertex normal
		GLfloat len = sqrt(pVtxNorm[COR_X] * pVtxNorm[COR_X] + pVtxNorm[COR_Y] * pVtxNorm[COR_Y] + pVtxNorm[COR_Z] * pVtxNorm[COR_Z]);
		pVtxNorm[COR_X] /= len;
		pVtxNorm[COR_Y] /= len;
		pVtxNorm[COR_Z] /= len;
	}
...
void calcVertexColor(float* pCoordinate, float* pResult)
{
	...
	h = (pCoordinate[COR_Z] - 37.0f) / (126.0f - 37.0f) * 256.0f;

	// color of a vertex given in the RGB space
	float r, g, b;

	// Transform the color into the RGB-space
	hsv2rgb(h, s, v, &r, &g, &b);

	// Set the color for the material (used with lighting model)
	pResult[0] = r;
	pResult[1] = g;
	pResult[2] = b;
...
bool loadMeshObject(const string & meshFileName)
{
	...
	for (int triIndex = 0; triIndex < numTriangles; triIndex++)
	{
		...
		GLfloat* pTriNorm = pTriangleArray[triIndex].normal;
		pTriNorm[COR_X] = vec1[COR_Y] * vec2[COR_Z] - vec1[COR_Z] * vec2[COR_Y];
		pTriNorm[COR_Y] = vec1[COR_Z] * vec2[COR_X] - vec1[COR_X] * vec2[COR_Z];
		pTriNorm[COR_Z] = vec1[COR_X] * vec2[COR_Y] - vec1[COR_Y] * vec2[COR_X];

		// Normalize the normal vector:
		GLfloat len = sqrt(pTriNorm[COR_X] * pTriNorm[COR_X] + pTriNorm[COR_Y] * pTriNorm[COR_Y] + pTriNorm[COR_Z] * pTriNorm[COR_Z]);
		pTriNorm[COR_X] /= len;
		pTriNorm[COR_Y] /= len;
		pTriNorm[COR_Z] /= len;
	}
	...
	assert(location != -1);
	glVertexAttribPointer(location, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), (GLvoid*)offsetof(Vertex, pos));
	...
	assert(location != -1);
	GLCheckError();
	glVertexAttribPointer(location, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), (GLvoid*)offsetof(Vertex, color));
	glEnableVertexAttribArray(location);
...
void displayFunc(void)
{
	...
	//===============================================================
	GLint location = glGetUniformLocation(g_shaderProgram, "InterestingPoint");
	assert(location != -1);
