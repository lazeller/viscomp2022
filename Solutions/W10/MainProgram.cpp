...
void MainProgram::Display(float deltaTime)
{
	...
	m_pShader->setUniform(m_pShader->getUniformLocation("time"), m_time);
	...
	posX = -sin(m_time * 0.7f) * 0.8f;
	posY = 0.0f;
	posZ = -cos(m_time * 0.8f) * 0.8f + 1.5f;
	...
	posX = 0.0f;
	posY = sin(m_time * 0.7f) * 0.8;
	posZ = cos(m_time * 0.7) * 0.8 + 1.5;
	renderObject(posX, posY, posZ);
...
void MainProgram::renderObject(float posX, float posY, float posZ)
{
	...
	m_pShader->setUniform(m_pShader->getUniformLocation("posObject"), posX, posY, posZ);
	// Activate the shader:
	m_pShader->use();
